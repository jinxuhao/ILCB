...
if __name__ == '__main__':
	...
	while not rospy.is_shutdown():
		...
		Cr,DK_data = dbscan(dataSet, 0.3, 3)# 0.1 20x_center_list_fina
		k=-1
		for i in range(len(rlist)):
			try:
				if k<Cr[i]:k=Cr[i]
		...
		if len(DK1x) !=0:
			...
			if len(x_center_0_list_final) >= 50:
		...
		if len(DK2x) !=0:
			x_center_1_list,y_center_1_list,R_1_list = set_circle_data(DK2x,DK2y)
			x_center_1 = x_center_1_list[0]
			y_center_1 = y_center_1_list[0]
			R_1_list = R_1_list[0]

			print('x-center',x_center_1_list[0])
			if len(x_center_1_list_final) >= 50:
		...
		if len(DK3x) !=0:
			x_center_2_list,y_center_2_list,R_2_list = set_circle_data(DK3x,DK3y)
			x_center_2 = x_center_2_list[0]
			y_center_2 = y_center_2_list[0]
			R_2_list = R_2_list[0]

		
			if len(x_center_2_list_final) >= 50:
			...
			# print('x_dian',p_0_2*time_now*time_now+p_0_1*time_now+p_0_0)#p_0_2*time_now*time_now+
			# print('x-center-zhong',x_center_0_list_final)
		...
		h1 = (math.pow((x-x_center_0), 2.) + math.pow((y-y_center_0), 2.) - math.pow((R_0_list+0.5), 2.))
		h2 = (math.pow((x-x_center_1), 2.) + math.pow((y-y_center_1), 2.) - math.pow((R_1_list+0.5), 2.))
		h3 = (math.pow((x-x_center_2), 2.) + math.pow((y-y_center_2), 2.) - math.pow((R_2_list+0.5), 2.))
		...
		h = matrix([(c*h1-2.*(x-x_center_0)*(p_0_0)-2.*(y-y_center_0)*(q_0_0))*p_0_flag,(c*h2-2.*(x-x_center_1)*(p_1_0)-2.*(y-y_center_1)*(q_1_0))*p_1_flag,(c*h3-2.*(x-x_center_2)*(p_2_0)-2.*(y-y_center_2)*(q_2_0))*p_2_flag])
		...
		msg.linear.x = vx*0.03#0.03*math.cos(Theta_err)
		msg.linear.y = vy*0.03#0.03*math.sin(Theta_err)
