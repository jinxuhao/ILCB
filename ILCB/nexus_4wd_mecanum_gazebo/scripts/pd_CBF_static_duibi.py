...
if __name__ == '__main__':
	...
	while not rospy.is_shutdown():
		...
		Cr,DK_data = dbscan(dataSet, 0.1,3)# 0.1 20x_center_list_fina
		k=-1
		for i in range(len(rlist)):
			try:
				if k<Cr[i]:k=Cr[i]
		...
		msg.linear.x = vx*0.052#0.4#0.23#0.03#0.05*math.cos(Theta_err)
		msg.linear.y = vy*0.052#0.4#0.23#0.03#0.05*math.sin(Theta_err)
